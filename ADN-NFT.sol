...
/// @custom:security-contact dev@aidream.network
contract AiDreamNetworkNFT is ERC721, ERC721Enumerable, AccessControl {
    using Counters for Counters.Counter;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Ai Dream Network NFT", "ADNFT") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
    }

    string _baseURIEditable = "https://adnft-uri.aidream.network/"; 
    
    function _baseURI() internal view override returns (string memory) {
        return _baseURIEditable;
    }

    function setBaseURI(string memory newURI) public onlyRole(DEFAULT_ADMIN_ROLE){
        _baseURIEditable = newURI;
    }
    ...
    function _burn(uint256 tokenId) internal override {
        ...
        if (bytes(tokenDataCID[tokenId]).length != 0) {
            delete tokenDataCID[tokenId];
        }
    }
